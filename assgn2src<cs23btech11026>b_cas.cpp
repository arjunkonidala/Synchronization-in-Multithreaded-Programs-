...
int p=0; int termi=0; 
atomic<int> u=0;                                                  //shared variable
int* flag;
int check_row(int x){
    int v[n];
    for(int i=0;i<n;i++){
...
void* f(void* arg){
    ...
    int turn ;
        ...
        flag[x->num-1]=1;
        turn =1;
        while(turn &&flag[x->num-1]){
            turn=u.exchange(1,memory_order_acquire);
        }
        //thread gets access
        ...
        access_time.push_back(d1.count());
        m.unlock();
        if(j==i)u.store(0,memory_order_release);
        else flag[j]=0;
        nav=p;
        ...
        for(int i=nav;i<nav+t&&i<3*n;i++){
            if((i)/n==0){
                ...
                time_t now = time(0);
                tm* ltm = localtime(&now);
                string st="Thread "+to_string(x->num)+" grabs row "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" + to_string(ltm->tm_min)+"hrs" ;
                m.lock();
                vec.push_back(st);
                m.unlock();
                if(!check_row(i%n)){
                    termi=1;
                    string st="Thread "+to_string(x->num)+" invalidates row "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" + to_string(ltm->tm_min)+"hrs";
                    m.lock();
                vec.push_back(st);
                m.unlock();
                    break;
                    ...
                    now = time(0);
                    string st="Thread "+to_string(x->num)+" validates row "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" + to_string(ltm->tm_min)+"hrs";
                    m.lock();
                vec.push_back(st);
                m.unlock();
                }
                ...
                time_t now = time(0);
                tm* ltm = localtime(&now);
                string st="Thread "+to_string(x->num)+" grabs column "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" + to_string(ltm->tm_min)+"hrs";
                ...
                if(!check_col(i%n)){
                    termi=1;
                    now = time(0);
                    string st="Thread "+to_string(x->num)+" invalidates column "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" +to_string(ltm->tm_min) +"hrs";
                    m.lock();
                vec.push_back(st);
                m.unlock();
                    break;
                    ...
                    now = time(0);
                    string st="Thread "+to_string(x->num)+" validates column "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" +to_string(ltm->tm_min) +"hrs";
                    m.lock();
                vec.push_back(st);
                m.unlock();
                }
                ...
                time_t now = time(0);
                tm* ltm = localtime(&now);
                string st="Thread "+to_string(x->num)+" grabs grid "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" + to_string(ltm->tm_min)+"hrs";
                m.lock();
                vec.push_back(st);
                m.unlock();
                if(!check_grid(i%n)){
                    termi=1;
                    now = time(0);
                    string st="Thread "+to_string(x->num)+" invalidates grid "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" + to_string(ltm->tm_min)+"hrs";
                    m.lock();
                vec.push_back(st);
                m.unlock();
                    break;
                    ...
                    now = time(0);
                    string st="Thread "+to_string(x->num)+" validates grid "+to_string(i%n)+" at "+to_string(ltm->tm_hour)+":" + to_string(ltm->tm_min)+"hrs";
                    m.lock();
                vec.push_back(st);
                m.unlock();
                }
